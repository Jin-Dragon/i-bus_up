...
function requireWorkerToken(request, response, next) {
  if (!config.workerSharedToken) {
    response.status(500).json({ ok: false, error: "WORKER_SHARED_TOKEN is not configured." });
    return;
  }

  const token = request.get("x-worker-token") || "";
  if (token !== config.workerSharedToken) {
...
app.get("/api/health", (_request, response) => {
  response.json({ ok: true, role: config.appRole });
});

app.get("/internal/worker/health", requireWorkerToken, (_request, response) => {
  response.json({ ok: true, role: config.appRole });
});

app.post("/internal/worker/login", requireWorkerToken, async (_request, response) => {
  try {
    await loginOneBus();
    response.json({ ok: true, message: "Worker login session saved." });
  } catch (error) {
    response.status(500).json({ ok: false, error: error.message });
  }
});

app.post("/internal/worker/publish", requireWorkerToken, async (request, response) => {
  try {
    const publishResult = await publishToOneBus(request.body.draft || {}, request.body.options || {});
    response.json({ ok: true, publishResult });
  } catch (error) {
    response.status(500).json({ ok: false, error: error.message });
  }
});
...
app.listen(port, () => {
  console.log(`Server running on http://localhost:${port} as ${config.appRole}`);
});
